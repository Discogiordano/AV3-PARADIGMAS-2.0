...
def main():
    ...
    if escolha == "Cadastrar Usuário":
        ...
        if st.button("Login"):
            ...
            st.write("Usuário logado:", st.session_state.usuario_logado)
        ...
        if st.session_state.usuario_logado:
            st.write("Conteúdos disponíveis:")
            conteudos_disponiveis = listar_conteudos(conteudos)
            if st.session_state.usuario_logado.tipo_assinatura == 'premium':
                conteudos_disponiveis += listar_conteudos(conteudos_2)

            for conteudo in conteudos_disponiveis:
                st.write(conteudo)
